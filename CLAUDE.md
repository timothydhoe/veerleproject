...
| **Validity criteria** | Sedentary analysis: ≥9 valid **waking** wear hours on ≥4 days (≥1 weekend), per Veerle's protocol document (`docs/info/info studie.docx`, citing Antczak et al. 2021) — matches `config.yaml`'s `min_wear_hours_per_day`/`min_valid_days`. "Waking" means 24h minus that night's GGIR-detected sleep period, **not** the full calendar day — GGIR's own `includedaycrit` (used elsewhere in `01_run_ggir.R` for GGIR's internal Part 2/5 processing) is a full-24h-day measure and does **not** drive this criterion; the pipeline's own validity gate (`03_build_summaries.R`'s `add_waking_valid_hours()`, shared with the dashboard via `utils_ggir.R`) subtracts each night's sleep-period time from Part 2's full-day valid hours instead. (A previous version of this table incorrectly stated this was a 24h calendar-day measure — see `docs/test/bug_log.md` #31, which supersedes #20's mischaracterization.) An earlier ≥16h/≥3-day figure (from an early email from Veerle, later superseded) still lingers in `docs/step2_ggir_pipeline_reference.md`'s historical quote. Sleep: ≥50% valid sleep on ≥5 nights. |
